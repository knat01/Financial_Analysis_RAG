...
def test_api():
    ...
    if not api_key:
        logger.error(
            "ALPHA_VANTAGE_API_KEY is not set in the environment variables.")
    ...
    try:
        ...
        for statement in ['income_statement', 'balance_sheet', 'cash_flow']:
            if statement not in financial_data:
                logger.error(
                    f"{statement} is missing from the financial data.")
                return f"The API response is missing {statement}. Please try again later."

            if 'annualReports' not in financial_data[
                    statement] and 'quarterlyReports' not in financial_data[
                        statement]:
